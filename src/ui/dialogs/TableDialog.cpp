...
LRESULT CALLBACK TableDialog::proc(HWND h, UINT m, WPARAM w, LPARAM l) {
  ...
  if (self && m == WM_COMMAND) {
    if (LOWORD(w) == IDOK)     { self->ok_ = true;  self->done_ = true; return 0; }
    if (LOWORD(w) == IDCANCEL) { self->ok_ = false; self->done_ = true; return 0; }
...
bool TableDialog::show(HWND parent, HFONT uiFont, int &cols, int &rows) {
  ...
  while (!done_) {
    ...
    if (msg.message == WM_KEYDOWN && msg.wParam == VK_RETURN) { ok_ = true; break; }
