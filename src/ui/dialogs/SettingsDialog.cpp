...
LRESULT CALLBACK SettingsDialog::proc(HWND h, UINT m, WPARAM w, LPARAM l) {
  ...
  if (self && m == WM_COMMAND) {
    ...
    if (id == IDOK)     { self->ok_ = true;  self->done_ = true; return 0; }
    if (id == IDCANCEL) { self->ok_ = false; self->done_ = true; return 0; }
    ...
    self->ok_ = false; self->done_ = true; return 0;
  }
...
bool SettingsDialog::show(HWND parent, HFONT uiFont, Settings &s) {
  ...
  for (auto v : { u8"편집만", u8"분할", u8"미리보기만" }) SendMessageW(cbView, CB_ADDSTRING, 0, (LPARAM)W(v).c_str());
  SendMessageW(cbView, CB_SETCURSEL, comboIndex(s.defaultView, kViews, 3, 1), 0);
  ...
  for (auto v : { u8"시스템", u8"라이트", u8"다크" }) SendMessageW(cbTheme, CB_ADDSTRING, 0, (LPARAM)W(v).c_str());
  SendMessageW(cbTheme, CB_SETCURSEL, comboIndex(s.theme, kThemes, 3, 0), 0);

  mk(L"STATIC", W(u8"글꼴 크기 (10-32)"), SS_LEFT, lx, 90, 120, 18, -1);
  HWND eFont = mk(L"EDIT", std::to_wstring(s.fontSize), ES_NUMBER | WS_BORDER | WS_TABSTOP, cx, 86, 60, 24, 2003);

  mk(L"STATIC", W(u8"탭 크기 (1-8)"), SS_LEFT, lx, 124, 120, 18, -1);
  HWND eTab = mk(L"EDIT", std::to_wstring(s.tabSize), ES_NUMBER | WS_BORDER | WS_TABSTOP, cx, 120, 60, 24, 2004);
  ...
  SendMessageW(ckSync, BM_SETCHECK, s.scrollSync ? BST_CHECKED : BST_UNCHECKED, 0);
  ...
  done_ = false; ok_ = false;
  MSG msg;
  while (!done_) {
    BOOL r = GetMessageW(&msg, nullptr, 0, 0);
    if (r == 0) { PostQuitMessage((int)msg.wParam); break; }
    if (msg.message == WM_KEYDOWN && msg.wParam == VK_ESCAPE) { ok_ = false; break; }
    if (msg.message == WM_KEYDOWN && msg.wParam == VK_RETURN) {
      ...
      } else { ok_ = true; break; }      // 그 외(편집/체크박스/리스트)는 저장
    }
  ...
  if (ok_) {
    ...
    s.theme = kThemes[ti];
    s.fontSize = dlgClamp(dlgReadInt(eFont, s.fontSize), 10, 32);
