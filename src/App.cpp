...
void App::wireCallbacks() {
  editor_.onTableNav   = [this](bool s) { return tableEditor_.nav(editor_.hwnd(), s); };
  editor_.onTableEnter = [this]() { return tableEditor_.enter(editor_.hwnd()); };
  editor_.onScroll     = [this]() { requestScrollSync(); };
  preview_.onReady     = [this]() { pushPreviewConfigAndRender(); };
  preview_.onOpenExternal = [this](const std::string &req) { return openExternal(req); };
}
...
bool App::launchVSCode(const std::wstring &file) {
  ...
  auto tryExe = [&](const std::wstring &exe) -> bool {
    ...
    return (INT_PTR)ShellExecuteW(hwnd_, L"open", exe.c_str(),
...
std::string App::openExternal(const std::string &req) {
  ...
  if (!startsWith("http://") && !startsWith("https://") && !startsWith("mailto:"))
    return "{\"ok\":false}";
  ShellExecuteW(hwnd_, L"open", utf8_to_wide(url).c_str(), nullptr, nullptr, SW_SHOWNORMAL);
  return "{\"ok\":true}";
}
...
void App::pushPreviewConfigAndRender() {
  preview_.pushConfig(theme_.isDark(), settings_.langsJson, curDir_, settings_.zoom);
...
void App::layout() {
  ...
  topbar_.layout(hwnd_, Wd, uiFont_.get());
  dividerX_ = -1;
  HWND edit = editor_.hwnd(), prev = preview_.host();
  if (view_ == 0) {              // 에디터만
    MoveWindow(edit, 0, top, Wd, ch, TRUE); ShowWindow(edit, SW_SHOW);
    if (prev) ShowWindow(prev, SW_HIDE);
  } else if (view_ == 2) {       // 미리보기만
    ShowWindow(edit, SW_HIDE);
    if (prev) { MoveWindow(prev, 0, top, Wd, ch, TRUE); ShowWindow(prev, SW_SHOW); }
  } else {                        // 분할
    int ew = (int)(Wd * splitRatio_), minw = 120;
    if (ew < minw) ew = minw;
    if (ew > Wd - minw - kDividerW) ew = Wd - minw - kDividerW;
    if (ew < 0) ew = 0;
    MoveWindow(edit, 0, top, ew, ch, TRUE); ShowWindow(edit, SW_SHOW);
    int px = ew + kDividerW;
    if (prev) { MoveWindow(prev, px, top, Wd - px, ch, TRUE); ShowWindow(prev, SW_SHOW); }
    dividerX_ = ew;
  }
...
void App::setView(int v) {
  view_ = v;
  if (v != 0) preview_.ensureEngine(editor_.hwnd()); // 에디터 전용이 아니면 엔진 지연 생성
  layout();
  refreshPreview();
  SetFocus(editor_.hwnd());
}
...
void App::applySettingsChange(const Settings &before) {
  ...
  if (settings_.theme != before.theme) {             // 테마: 브러시 재생성 + 리페인트
    theme_.setMode(settings_.theme);
    editBrush_.reset(CreateSolidBrush(theme_.bg()));
    InvalidateRect(editor_.hwnd(), nullptr, TRUE);
    InvalidateRect(hwnd_, nullptr, FALSE);
  }
  if (settings_.theme != before.theme || settings_.langsJson != before.langsJson)
    refreshPreview();
  settings_.save();
}
...
void App::runBtn(int id) {
  switch (id) {
    ...
    case IDM_VSCODE: openInVSCode(); break;
    case IDM_INSERTTABLE: {
      int c, r;
      if (tableDialog_.show(hwnd_, uiFont_.get(), c, r)) tableEditor_.insertSkeleton(editor_.hwnd(), c, r);
      SetFocus(editor_.hwnd());
      break;
    }
    case IDM_FORMATTABLE: SetFocus(editor_.hwnd()); tableEditor_.formatTable(editor_.hwnd()); break;
    case IDM_SETTINGS: {
      Settings before = settings_;
      if (settingsDialog_.show(hwnd_, uiFont_.get(), settings_)) applySettingsChange(before);
      SetFocus(editor_.hwnd());
      break;
    }
    case IDM_ZOOM_IN:    setZoom(settings_.zoom + kZoomStep); break;
    case IDM_ZOOM_OUT:   setZoom(settings_.zoom - kZoomStep); break;
    case IDM_ZOOM_RESET: setZoom(100); break;
    case IDM_MIN:    ShowWindow(hwnd_, SW_MINIMIZE); break;
    case IDM_MAX:    ShowWindow(hwnd_, IsZoomed(hwnd_) ? SW_RESTORE : SW_MAXIMIZE); break;
    case IDM_WCLOSE: SendMessageW(hwnd_, WM_CLOSE, 0, 0); break;
...
LRESULT App::onMessage(HWND h, UINT m, WPARAM w, LPARAM l) {
  switch (m) {
    ...
    case WM_NCHITTEST: {
      ...
      if (cp.y < kTopbarH) return topbar_.btnAt(cp.x, cp.y) >= 0 ? HTCLIENT : HTCAPTION;
      return HTCLIENT;
    }
    case WM_ERASEBKGND:
    ...
    case WM_PAINT: {
      ...
      topbar_.paint(dc, cr.right, theme_, uiFont_.get(), glyphFont_.get(),
                    dirty_, name, view_, IsZoomed(h) != 0);
      if (view_ == 1 && dividerX_ >= 0) { // 디바이더 스트립
        RECT dv = { dividerX_, kTopbarH, dividerX_ + kDividerW, cr.bottom };
        BrushHandle b(CreateSolidBrush(theme_.border())); FillRect(dc, &dv, (HBRUSH)b.get());
      }
      EndPaint(h, &ps);
      return 0;
    }
    case WM_LBUTTONDOWN: {
      ...
      if (view_ == 1 && dividerX_ >= 0 && mx >= dividerX_ && mx < dividerX_ + kDividerW && my >= kTopbarH) {
        SetCapture(h); divDrag_ = true; return 0;
      }
    ...
    case WM_MOUSELEAVE:
      if (topbar_.hot() != -1) { topbar_.setHot(-1); invalidateTopbar(); }
      return 0;
    case WM_LBUTTONUP:
      if (divDrag_) { divDrag_ = false; ReleaseCapture(); }
      return 0;
    case WM_SETCURSOR:
      if (LOWORD(l) == HTCLIENT && view_ == 1 && dividerX_ >= 0) {
        POINT p; GetCursorPos(&p); ScreenToClient(h, &p);
        if (p.x >= dividerX_ && p.x < dividerX_ + kDividerW && p.y >= kTopbarH) {
    ...
    case WM_APP_PREWARM:
      ...
      return 0;
    case WM_TIMER:
      if (w == IDT_RENDER) { KillTimer(h, IDT_RENDER); preview_.pushRender(editor_.getTextUtf8Lf()); }
    ...
    case WM_COMMAND: {
      if ((HWND)l == editor_.hwnd() && HIWORD(w) == EN_CHANGE) {
        if (!editor_.isSuppressing()) {
          if (!dirty_) { dirty_ = true; updateTitle(); }
          if (view_ != 0) SetTimer(h, IDT_RENDER, 120, nullptr); // 프리뷰 디바운스
        }
        return 0;
      }
      switch (LOWORD(w)) {
        case IDM_NEW:    runBtn(IDM_NEW);    return 0;
        case IDM_OPEN:   runBtn(IDM_OPEN);   return 0;
        case IDM_SAVE:   runBtn(IDM_SAVE);   return 0;
        case IDM_SAVEAS: runBtn(IDM_SAVEAS); return 0;
        case IDM_VSCODE: runBtn(IDM_VSCODE); return 0;
    ...
    case WM_CLOSE:
      ...
      return 0;
    case WM_DESTROY:
      preview_.destroyEngine(); // 호스트 창이 살아 있을 때 WebView2 컨트롤러 정리
      PostQuitMessage(0);
      return 0;
  }
...
int App::run() {
  ...
  settings_.load();
  theme_.setMode(settings_.theme);
  view_ = (settings_.defaultView == "editor") ? 0 : (settings_.defaultView == "preview") ? 2 : 1;
  editBrush_.reset(CreateSolidBrush(theme_.bg()));
  uiFont_ = FontHandle(CreateFontW(-13, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                       OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
                       VARIABLE_PITCH | FF_SWISS, L"Segoe UI"));
  glyphFont_ = FontHandle(CreateFontW(-15, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                          OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
  ...
  while (GetMessageW(&msg, nullptr, 0, 0)) {
    if (!TranslateAcceleratorW(hwnd_, hAccel, &msg)) {
