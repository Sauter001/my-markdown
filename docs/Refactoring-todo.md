...
- 진행: A1/A2(앱 객체 + 모듈 분리), A3, C6(디스패치 통합), C1(Settings 모델/언어 벡터화), B1(Command 테이블 완전 통합)은 완료. 이후 에디터가 RichEdit로 전환되며 신규
  후보(D 섹션)가 생겼다. `main.cpp`는 현재 진입점만(약 13줄) 남았다.
...
| P1 | 액션을 Command 테이블로 통합 (완료)           | B   | 메타데이터 단일 출처(`commandTable`)에서 버튼/액셀/디스패치 생성       |
...
| P1 | 액션 Command 테이블 (B1) (완료)         | B   | 디스패치 + 버튼/액셀/툴팁 메타데이터까지 단일 출처화 완료                |
...
### B1. Command 테이블 (P1, 가장 효과 큼) - 완료(2026-06-23)
...
- 현황 갱신(부분 완료, 2026-06-22): 디스패치를 단일화했다. `App`에 `struct Command { int id; std::function<void()> run; }` 레지스트리(`commands_`,
  ...
  단일 출처) 조회로 대체됐다. 세 인보커(상단바/액셀/미리보기) 모두 동일 `IDM_*`로 레지스트리를 통해 디스패치된다.
- 완전 통합(완료, 2026-06-23): 메타데이터를 `commands.h`/`commands.cpp`의 `commandTable()`(`struct CommandInfo`: idm, actionId, label, defShortcut, glyph, tip,
  btn) 단일 출처로 모았다. `keymap::actions()`는 손수 유지하던 리터럴 목록 대신 `commandTable()`의 `actionId` 보유 항목에서 파생되고(어댑터), `Topbar::layout`은
  인라인 글리프/툴팁 리터럴 대신 `findCommand(idm)`로 메타를 채워 버튼을 생성한다(`TopBtn::type`은 `BtnType` enum으로 전환, 위치 계산 루프는 회귀 위험으로 불변 유지).
  실행 핸들러는 `this` 캡처가 필요해 `App::buildCommands()`에 남는다(디스패치는 이미 단일화). 이제 액션의 라벨/단축키/글리프/툴팁/종류는 `commandTable()` 한 곳에서만
  관리된다(핸들러만 App에 추가).
...
- 적용 결과(완료): `WM_COMMAND`가 모든 명령 ID를 `runBtn`으로 위임한다(인라인 중복 제거). 이후 B1 디스패치 단일화(2026-06-22)로 `WM_COMMAND`의 위임 전용 switch가
  `runBtn(LOWORD(w))` 한 줄이 되고 `runBtn`은 Command 레지스트리 조회로 바뀌었다(B1 항목 참고). 상단바 버튼/액셀러레이터 테이블 생성까지의 완전 통합도 B1으로 완료됐다(2026-06-23).
