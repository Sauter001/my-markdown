...
간이 마크다운 에디터. Windows 네이티브(Win32) 애플리케이션으로, 좌측 편집은 네이티브 EDIT 컨트롤, 우측 렌더링은 지연 임베드한 WebView2 미리보기로 처리한다. 메모장처럼 즉시 로딩되어 편집할
수 있다.
...
- 괄호/따옴표 자동 페어링은 네이티브 에디터에 아직 이식되지 않았다(구 web UI 기능).
