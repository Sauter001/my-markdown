...
간이 마크다운 에디터. Windows 네이티브(Win32) 애플리케이션으로, 좌측 편집은 네이티브 EDIT 컨트롤, 우측 렌더링은 지연 임베드한 WebView2 미리보기로 처리한다. 메모장처럼 즉시 로딩되어 편집할 수 있다.
...
```
MyMD/
  src/main.cpp        네이티브 호스트 (창/상단바, 에디터 EDIT, 표/리스트 편집, 대화상자, 파일 입출력, 설정)
...
### 동작 개요
- 네이티브 호스트가 UI를 소유한다. 좌측은 EDIT 컨트롤(에디터), 우측 패널에만 WebView2 미리보기를 지연 임베드한다(에디터 전용 보기에서는 미리보기를 만들지 않아 즉시 로딩).
- 호스트 -> 미리보기는 `g_webview->eval`로 `window.mymd*`(렌더/테마/base/언어/스크롤)를 호출하고, 동적 문자열은 base64로 전달한다. 미리보기 -> 호스트는 준비 통지와 외부 링크 열기만 `bind` 한다.
- 파일 저장은 네이티브 입출력이라 Ctrl+S가 같은 파일에 확실히 기록된다.

## 알려진 한계 / 향후 작업
- 미리보기 코드 블록은 Prism으로 강조하지만, 입력 영역(에디터)은 네이티브 EDIT 컨트롤이라 강조가 없다(즉시 로딩 우선).
- 단축키 재바인딩(keymap)은 미구현이며 현재 단축키는 고정값이다.
